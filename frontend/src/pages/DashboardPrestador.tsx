...
export const DashboardPrestador: React.FC = () => {
  ...
  const fetchProviderData = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiService.getProvider(profile.id);
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProviderDetails((prev) => prev ? { ...prev, business_name: businessName, description, center_lat: lat, center_lng: lng, radius_km: radius } : prev);
    ...
    <div style={styles.page} className="fade-in">
      <div className="container" style={styles.container}>
        ...
        <div style={styles.metricsRow}>
          ...
          />
          <MetricCard
            icon={<Zap size={18} color="var(--info)" />}
            value="R$ 20/mês"
            ...
            <div style={styles.card}>
              ...
              {!providerDetails?.recent_reviews?.length ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {providerDetails.recent_reviews.map((rev: Review) => (
